//Данные о супергероях в формате JSON
const dataJSON = `[
    {
      "name": "Бетмен",
      "universe": "DC Comics",
...
const superheroes = JSON.parse(dataJSON);

//получаем доступ к главному контейнеру о героях
const cards = document.querySelector(".cards");

//функция отрисовки карточки о супергероях
const createSuperheroCard = (obj) => {
  //создаем контейнер для карточки о супергерое
  const card = document.createElement("div");
  card.className = "card__superhero";

  //создаем контейнер для имени супергероя
  const name = document.createElement("h2");
  name.textContent = obj.name;
  card.appendChild(name);

  //создаем контейнер для Вселенной супергероя
  const universe = document.createElement("p");
  universe.textContent = `Universe: ` + obj.universe;
  card.appendChild(universe);

  //создаем контейнер для альтерэго супергероя
  const alterego = document.createElement("p");
  alterego.textContent = `Alterego: ` + obj.alterego;
  card.appendChild(alterego);

  //создаем контейнер для рода занятия супергероя
  const occupation = document.createElement("p");
  occupation.textContent = `Occupation: ` + obj.occupation;
  card.appendChild(occupation);

  //создаем контейнер для друзей супергероя
  const friends = document.createElement("p");
  friends.textContent = `Friends: ` + obj.friends;
  card.appendChild(friends);

  //создаем контейнер для сверхспособностей супергероя
  const superpowers = document.createElement("p");
  superpowers.textContent = `Superpowers: ` + obj.superpowers;
  card.appendChild(superpowers);

  //создаем контейнер для фото супергероя
  const image = document.createElement("img");
  image.className = "image__superhero";
  image.src = obj.url;
  card.appendChild(image);

  //создаем контейнер для информации о супергерое
  ...
  if (localStorage.getItem(formCreate.id)) {
    ...
    const oldStarImputs = formCreate.querySelectorAll(".icon");
    //закрашиваем звезды согласно значениям, полученным из localStorage
    for (let i = 0; i < savedScore; i++) {
      oldStarImputs[i].style = "fill: gold";
    }
  }
...
const addSuperheroCard = (objItem, container) => {
  const item = createSuperheroCard(objItem);
...
cards.addEventListener("click", (event) => {
  if (event.target.classList.contains("container__input")) {
    //получаем элемент c классом container__input, на котором произошел click
    const form = event.target.parentNode.parentNode; //получаем элемент родителя родителя form
    const inputs = form.querySelectorAll(".container__input"); //находим input
    const icons = form.querySelectorAll(".icon"); //находим icon
    const key = form.id; //находим ключ
    const value = event.target.value; //получаем значение value из input, по которому произоше клик

    //метод forEach для каждой звезды рейтинга, который закрашивает ее  в черный цвет, чтобы если пользователь изменит рейтинг иконка опять становилась черной
    icons.forEach((elem) => {
      elem.style = "fill: black";
    });

    //закрашиваем отмеченную иконку inputs[i].checked желтым цветом
    ...
    //записываем значение инпутов со свездами и ключ, по которым произошел клик в localStorage
